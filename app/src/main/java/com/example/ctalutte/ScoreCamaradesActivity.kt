...
class ScoreCamaradesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var listeCamarades = connexionBDD.getListeInfosCamarades(false)

        var listeTemp = arrayListOf<String>()

        for(Camarade in listeCamarades){
            Outils.logPerso("ListeCamaradeItem", Camarade.nomCamarade)
            listeTemp.add(Camarade.nomCamarade + ", score : "+Camarade.scoreCamarade + ", tâches terminées : " + Camarade.nb_taches + ", Statut final : " + Camarade.getEtat_partie())
        }
